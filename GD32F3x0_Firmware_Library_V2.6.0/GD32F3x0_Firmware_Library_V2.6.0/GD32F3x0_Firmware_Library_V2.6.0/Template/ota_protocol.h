/*!
    ...
    0x08002000 - 0x08008FFF  App 运行区 (28KB)
    0x08009000 - 0x0800FFFF  下载区 (28KB)
    0x08010000 - 0x0801FFFF  数据分区 (64KB) — OTA 升级不触碰
*/
...
#define OTA_HEAD            0xAAU   /* 请求帧头（上位机→设备） */
#define OTA_RSP_HEAD        0x55U   /* 回复帧头（设备→上位机，与主控 820 协议一致） */
...
/*
 * 升级标志存储在下载区最后一页（0x0800F800），
 * 与固件数据不冲突（固件最大 26KB，结束于 0x0800F7FF）。
 * Bootloader 启动时读取此地址的魔数，判断是否有待安装的固件。
 * 注意：0x0801F000 地址 Flash 写入可能失败，故改用下载区末尾。
 */
#define OTA_FLAG_ADDR       0x0800F800U  /* 升级标志存储地址（下载区最后一页） */
