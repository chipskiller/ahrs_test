/*!
    ...
    │ 0x08002000 - 0x08008BFF  App 运行区 (27KB) ← 你的固件       │
    │ 0x08008C00 - 0x0800F7FF  下载区 (27KB) ← OTA 临时存放新固件 │
    │ 0x0800F800 - 0x0800FFFF  数据分区 (2KB) ← 保存 yaw 等数据  │
    └─────────────────────────────────────────────────────────────┘
...
#define APP_END_ADDR                                                           \
  0x08008BFFU /* App 区结束地址（第 34 页），升级时会被擦除 */
#define DOWNLOAD_START_ADDR                                                    \
  0x08008C00U /* 下载区起始地址（第 35 页），临时存放新固件 */
#define DOWNLOAD_SIZE 0x00006C00U /* 下载区大小 27KB（第 35-61 页） */
#define DATA_ZONE_START                                                        \
  0x0800F800U /* 数据分区起始（第 62 页），OTA 绝不触碰              \
               */
